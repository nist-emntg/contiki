...
void add_to_parent_cache(nodeid_t* parent) {
	int i;
	for (i = 0;
			i < sizeof(AKM_DATA.parent_cache) / sizeof(AKM_DATA.parent_cache[0]);
			i++) {
...
void set_authentication_state(nodeid_t* node_id, authentication_state authState) {
	int i;
	for (i = 0; i < NELEMS(AKM_DATA.authenticated_neighbors); i++) {
		if (rimeaddr_cmp(&AKM_DATA.authenticated_neighbors[i].node_id, node_id)) {
			if (AKM_DATA.authenticated_neighbors[i].state != authState  &&
					authState == AUTH_PENDING) {
				schedule_pending_authentication_timer(node_id);
			} else if ( AKM_DATA.authenticated_neighbors[i].state != authState  &&
					authState == OK_SENT_WAITING_FOR_ACK) {
				schedule_pending_authentication_timer(node_id);
			} else if ( AKM_DATA.authenticated_neighbors[i].state != authState  &&
					authState == CHALLENGE_SENT_WAITING_FOR_OK) {
...
void handle_auth_challenge_response(nodeid_t* sender_id,
	...
	if (!sec_verify_auth_response(pacr)) {
		...
		if (AKM_DATA.is_dodag_root) {
			if (is_capacity_available()) {
				add_authenticated_neighbor(sender_id, &pacr->session_key,
						AUTHENTICATED);
			...
			if (pacr->continuation == AUTH_CONTINUATION_NONE) {
				...
				if (is_capacity_available()) {
					...
					send_break_security_association(pparent,
							BSA_CONTINUATION_NONE, 0);
			...
			} else if (pacr->continuation == AUTH_CONTINUATION_INSERT_NODE) {

				nodeid_t bsaNode = pacr->coninuation_state1;
				nodeid_t continuationState = pacr->continuation_state;
				send_break_security_association(&bsaNode,
						BSA_CONTINUATION_INSERT_NODE, &continuationState);
			}
		}
	}
...
void handle_auth_ack(nodeid_t* sender, auth_ack_t* pauthAck) {
	...
	if (is_nodeid_zero(pparent)) {
		if ( get_authentication_state(sender) == OK_SENT_WAITING_FOR_ACK ) {
			...
			add_authenticated_neighbor(sender,&pauthAck->session_key,AUTHENTICATED);
		}
	} else {
...
void send_auth_ack(nodeid_t* target_id,nodeid_t * pparent) {
	sec_generate_session_key();
	if (AKM_DATA.is_dodag_root) {
		akm_send(target_id, AUTH_ACK,
				sizeof(akm_mac_header_t)
						+ sizeof(AKM_MAC_OUTPUT.data.auth_ack));
	} else {
		if ( pparent == NULL){
			memset(&AKM_MAC_OUTPUT.data.auth_ack.parent_id,0,sizeof(nodeid_t));
		} else  {
			rimeaddr_copy(&AKM_MAC_OUTPUT.data.auth_ack.parent_id,
										pparent);
		}
		akm_send(target_id, AUTH_ACK,sizeof(akm_mac_header_t)
								+ sizeof(AKM_MAC_OUTPUT.data.auth_ack));
	}
}
...
void schedule_send_challenge_timer(int time, nodeid_t* target) {
	...
	clock_time_t ctime = time*CLOCK_SECOND;
	ctimer_set(&AKM_DATA.send_challenge_delay_timer[i], ctime, do_send_challenge, target);
}
...
void schedule_challenge_sent_timer(nodeid_t *target)
{
	...
	if (i != -1) {
		clock_time_t time = CHALLENGE_SENT_TIMEOUT * CLOCK_SECOND;
		if ( !ctimer_expired(&AKM_DATA.auth_timer[i]) ){
			ctimer_stop(&AKM_DATA.auth_timer[i]);
		}
		ctimer_set(&AKM_DATA.auth_timer[i],time , handle_challenge_sent_timeout, target);
	}
}
